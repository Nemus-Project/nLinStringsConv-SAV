...
hTypeLong = false;          % selects longitudinal grid spacing
...
V = + 0.5*T0*h*(qMid.'*qMid) + 0.5*T0*h*(pMid.'*pMid) - 0.5*E*I*h*(qMid.'*DxxPr*qMid)+ facNlin*sum((sqrt((1 + pMid).^2 + qMid.^2) - 1).^2);
...
for n = 1 : timeSamples
    ...
    if SAV
    %%%% SAV 
        Vtemp = + 0.5*T0*h*(q.'*q) + 0.5*T0*h*(p.'*p) - 0.5*E*I*h*(q.'*DxxPr*q) + facNlin*sum((sqrt((1 + p).^2 + q.^2) - 1).^2);
        V = sqrt(2*Vtemp + shiftV);
        
        pot = sqrt( (1 + p).^2 + (q).^2);
        g1 = T0*h*q - E*I*h*DxxPr*q + 2*facNlin*(pot - 1).*q./pot;
        g2 = T0*h*p + 2*facNlin*(pot - 1).*(1+p)./pot;
