...
omega2 = 1000*2*pi;
T601 = 15; T602 = 10;
param1 = (-c_trasv^2 + sqrt(c_trasv^4 + 4*K^2*omega1^2))/(2*K^2);
param2 = (-c_trasv^2 + sqrt(c_trasv^4 + 4*K^2*omega2^2))/(2*K^2);
...
h1 = sqrt((c_trasv^2*k^2 + 4*sigma1*k + sqrt((c_trasv^2*k^2 + 4*sigma1*k)^2+((16*K^2*k^2))))/2)*(1 + epsilon);
h2  = sqrt(E / rho) * k ;
...
if hTypeLong
    ...
    k = 0.999*(-(h^2*(2*sigma1 + 2*epsilon*sigma1 - (4*K^2*epsilon^2 + 8*K^2*epsilon + 4*K^2 + c_trasv^2*h^2 + 4*epsilon^2*sigma1^2 + 8*epsilon*sigma1^2 + 4*sigma1^2)^(1/2)))/(4*K^2*epsilon^3 + 12*K^2*epsilon^2 + 12*K^2*epsilon + 4*K^2 + c_trasv^2*epsilon*h^2 + c_trasv^2*h^2));
    SR = floor(1/k);
    timeSamples = floor(SR*durSec);
end
nPoints = N - 1;
...
p  = Dxm*vPrev;
pot = sqrt( (1 + p).^2 + (q).^2);
nlin = [Dxp*(2*facNlin*(pot - 1).*(q - q./pot)); Dxp*(2*facNlin*(pot - 1).*(1 - (1+p)./pot))];
...
V = 0 ;
for n = 1 : N
    pMid = 0.5*( p(n)+pPrev(n) );
    qMid = 0.5*(q(n) + qPrev(n));
    pot = sqrt((1 + pMid )^2 + qMid^2);
    V = V + 0.5*qMid^2 + 3/2 + pMid - pot;
end
V = facNlin*V;
...
for n = 1 : timeSamples
    ...
    %%%% SAV 
        pot = sqrt((1 + p).^2 + (q).^2);
        g1 = facNlin*(q - q./pot);
        g2 = facNlin*(1 - (1+p)./pot);
        Vtemp = facNlin*sum(0.5*q.^2 + p + 3/2 - pot);
        ...
        if computeEnergy
            ...
            PotLin(n) = - 0.5*T0*h*uNext.'*(Dxx*u) - 0.5*E*Area*h*vNext.'*(Dxx*v) + 0.5*E*I*h*(Dxx*uNext).'*(Dxx*u);
    ...
    %%%% StormerVerlet 
        pot = sqrt( (1 + p).^2 + (q).^2);
        nlin = [Dxp*(facNlin*(q - q./pot)); Dxp*(facNlin*(1 - (1+p)./pot))];
    ...
    if ~SAV
        ...
        pMid = 0.5*(p + pNext);
        pot = sqrt((1 + pMid ).^2 + qMid.^2);
        V = facNlin*sum(0.5*qMid.^2 + 3/2 + pMid - pot);
        psiNext = sqrt(2*V + shiftV);
        Psi(n+1) = psiNext;
        lala1 = sum(0.5*q.^2 + p + 1);
        lala2 = sum(sqrt((p+1).^2 + q.^2));
