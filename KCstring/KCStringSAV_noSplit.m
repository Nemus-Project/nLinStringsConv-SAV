%++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%    Kirchhoff-Carrier string SAV non split form
%                    Riccardo Russo
...
u = uPrev + 0.5*k^2*(h*c^2*Dxx*uPrev - h*K^2*Dxxxx*uPrev + h*(0.5*E*h/(L*rho))*(-uPrev.'*Dxx*uPrev)*Dxx*uPrev);
...
V = - 0.5*T0*h*uMid.'*(Dxx*uMid) + 0.5*E*I*h*(Dxx*uMid).'*(Dxx*uMid) + (E*h^2*Area/8/L)*(-uMid.'*Dxx*uMid)^2;
...
for n=1:timeSamples
    ...
    %%%% SAV 
        lin = rA*(2*u - uPrev);
        Vtemp = - 0.5*T0*h*u.'*(Dxx*u) + 0.5*E*I*h*(Dxx*u).'*(Dxx*u) + (E*h^2*Area/8/L)*(-u.'*Dxx*u)^2;
        V = sqrt(2*(Vtemp + shiftV));
        g = h*T0*Dxx*u - h*E*I*Dxxxx*u + 0.5*E*Area*h^2/L*(-u.'*Dxx*u)*Dxx*u;
        g = -g/V;
