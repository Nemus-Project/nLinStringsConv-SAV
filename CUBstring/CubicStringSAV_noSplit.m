%++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%             Cubic string SAV non split form
%                    Riccardo Russo
...
r = (Dxm*uMid).^2;
norm = h*(r.'*r);
V = - 0.5*T0*h*uMid.'*(Dxx*uMid) + 0.5*E*I*h*(Dxx*uMid).'*(Dxx*uMid)+  norm*(E*Area - T0)/8;
...
for n=1:timeSamples
    ...
    %%%% SAV 
        ...
        r = (Dxm*u);
        norm = h*(r.*r).'*(r.*r);
        V = - 0.5*T0*h*u.'*(Dxx*u) + 0.5*E*I*h*(Dxx*u).'*(Dxx*u) + nLinOn*norm*(E*Area - T0)/8;
        coeff = 1/sqrt(2*(V + shiftV));
        g = h*T0*Dxx*u - h*E*I*Dxxxx*u + nLinOn*h*Dxp*(r.*r.*r)*(E*Area - T0)/2;
        g = -g*coeff;
